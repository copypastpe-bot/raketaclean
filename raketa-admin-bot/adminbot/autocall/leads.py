"""Отбор заявок с сайта: какая сделка наша и по какому номеру звонить.

Сайт вешает на сделку тег «Заявка с сайта» — по нему autocall отличает свои
заявки от лидов, заведённых руками на том же этапе. Телефон достаём из
контактов сделки: у сделки контактов может быть несколько, главный помечен
флагом is_main.
...
def is_site_lead(lead: Optional[Mapping[str, Any]]) -> bool:
    """Наша ли это заявка: есть ли на сделке тег «Заявка с сайта».

    Тег в амо правят руками, поэтому сравниваем без регистра и без краевых
    пробелов — «заявка с сайта» тоже считается. Часть имени не считается:
    просто «Заявка» — это другой тег.
    """
    wanted = SITE_TAG.casefold()
    return any(name.strip().casefold() == wanted for name in lead_tag_names(lead))
