...
«менеджер → отбивка → клиент» и спросить, чем он закончился. Как АТС это
делает внутри — не касается ни машины переходов (chain.py), ни хранилища
...
её API разведаны вживую на боевой АТС (Задача 7, 2026-08-31).
"""
...
class OnlinePbx:
    ...
    async def call_now(self, *, to_dial: str, client_phone: str) -> str:
        """Запустить звонок «менеджер(to_dial) → отбивка → клиент», вернуть id попытки.

        `POST call/now.json` JSON `{"from": to_dial, "to": "7"+client_phone}`
        (формат тела разведан на стенде: пустое тело даёт «body must be
        object»). Формат ОТВЕТА не разведан — может прийти uuid попытки,
        может не прийти ничего полезного. Если в `data` нашёлся uuid/id —
        используем его; иначе строим синтетический id
        `search:{телефон}:{unix-время старта}` — по нему `call_outcome`
        найдёт попытку в истории через телефон и время, раз готового id не
        досталось. Синтетический id хранит телефон ЦЕЛИКОМ (без маски) —
        это не лог, а рабочие данные для последующего поиска; в логи телефон
        уходит только замаскированным (`adminbot.phone.mask`).
        """
        ...
        if call_id is None:
            call_id = f"{_SEARCH_PREFIX}{client_phone}:{int(started_at.timestamp())}"
            log.info(
