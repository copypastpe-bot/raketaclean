...
class DeletionHandler:
    ...
    шагом своего тика (решение владельца 4). Один и тот же объект подключён
    сразу к ДВУМ независимым наблюдателям — заказам и уборкам (`main.py`), у
    каждого свой таймер, и `run()` может быть вызван почти одновременно из
    обоих. Без взаимного исключения оба вызова успевали бы прочитать одну и ту
    же неразобранную запись раньше, чем первый поставит отметку, и примечание
    с письмом владельцу уходили бы дважды на одно удаление (ревью 17.09,
    замечание 1 — воспроизведено `asyncio.gather(handler.run(), handler.run())`
    на живом Postgres). Поэтому `run()` целиком идёт под собственным
    `asyncio.Lock`: список неразобранных записей читается уже под ним, и
    второй вошедший видит отметки первого — повторно ничего не обрабатывает.
    ...
    async def run(self) -> int:
        ...
        Весь проход — под `self._lock` (см. докстринг класса): вызов из двух
        наблюдателей почти одновременно не должен разбирать одну и ту же
        запись дважды. Список читается уже под замком, поэтому второй вошедший
        видит отметки первого.
        """
