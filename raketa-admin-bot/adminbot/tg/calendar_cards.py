...
def cancellation_card(link: Any, *, dry_run: bool = False) -> tuple[str, InlineKeyboardMarkup]:
    ...
    if question.get("child_lookup_failed"):
        # CRM не ответила — либо на поиске дочки, либо на проверке её статуса
        # (ревью 22.09, задача 4): статус не подтверждён, показывать вместо
        # него лид воронки 1 нельзя, это его и увело бы в закрытие. Уточним
        # при подтверждении.
        confirm_line = ("CRM не ответила, проверю сделку при закрытии. "
                        "Закрыть заказ как несостоявшийся?")
    else:
