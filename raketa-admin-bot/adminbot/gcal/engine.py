...
def _compare_contact(record_name: Optional[str], record_phone10: Optional[str],
                     contact: Optional[dict]) -> Optional[bool]:
    """Сверка «номер + имя» — ядро для `contact_mismatch_text`/`contact_resolved`.

    `None` — сверять было нечем: контакта нет, в записи нет имени, у контакта
    имя автоматическое. `True` — пара сошлась. `False` — расхождение.
    """
    if contact is None:
        return None
...
def contact_mismatch_text(record_name: Optional[str], record_phone10: Optional[str],
    ...
    `None` — сверять было нечем, или пара сошлась: в обоих случаях расхождение
    ставить не в что, поэтому вызывающая сторона просто ничего не пишет —
    отличать «нечем» от «сошлось» в местах, которые только ПИШУТ расхождение
    (три точки в движке), не нужно. Строка — расхождение человеческим языком.
...
def contact_resolved(record_name: Optional[str], record_phone10: Optional[str],
                     contact: Optional[dict]) -> bool:
    """True — контакт есть, и он ТОЧНО сходится с записью (не «нечем сверить»).

    Отдельно от `contact_mismatch_text`: циклу напоминаний (`gcal/contact_reminder.py`)
    нужно различать «сошлось — можно снять расхождение и замолчать» от
    «сверить сейчас нечем — молчать рано, напоминание идёт как раньше»
    (контакт временно не читается, дочки ещё нет). Оба случая дают `mismatch
    is None`, но останавливать цикл должен только первый.
    """
    return _compare_contact(record_name, record_phone10, contact) is True
