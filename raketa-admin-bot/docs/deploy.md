...
Календарей может быть несколько (уборки бригадир ведёт в своём): они перечисляются
в `GCAL_CALENDAR_ID` через запятую **без пробелов**, первым — основной. Как выдать
доступ и что проверить — `docs/gcal_access.md`, раздел «Как добавить ещё один
календарь». У каждого календаря своя закладка обмена (миграция 008), поэтому один
не стирает изменения другого.
...
шаг 2: стенд с АТС (после появления боевого ключа — Задача 7 плана autocall):
       тестовая цепочка, где «клиент» — номер владельца; проверить отбивку,
