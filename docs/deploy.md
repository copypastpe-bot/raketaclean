...
- `PROMO_INTEREST_ADMIN_MESSAGE`: `1` (по умолчанию, если ключа нет) — админам уходит старое сообщение «откликнулся на промо», `0` — не уходит. Заявка на звонок в `promo_callbacks` пишется при любом значении. `0` ставится последним шагом выката автозвонка по промо (ТЗ `docs/plans/2026-09-23-promo-autocall.md`, «Порядок выката», шаг 6), откат — `1`; рестарт рабочего бота по слову владельца.
