...
class JournalAdapter:
    ...
    async def _collect(self, now: datetime) -> dict[tuple, dict]:
        ...
        ПД и сгруппировать одинаковые (в рамках этого прохода) под одним
        ключом — счётчик `count` уже здесь ловит залповые повторы (проверка
        ТЗ: 100 одинаковых ошибок сразу -> одна строка со счётчиком)."""
        ...
        for unit, source in self.sources.items():
            ...
            for entry in entries:
                ...
                key = (unit, parsed.module, parsed.level, masked)
                slot = batch.get(key)
                if slot is None:
