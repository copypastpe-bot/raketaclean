...
Перенесено без изменений из `adminbot/tg/session.py`: служба едет на сервер
отдельно от ботов и импортировать их код не может, поэтому приём дублируется.
Меняется там — меняется и здесь.
