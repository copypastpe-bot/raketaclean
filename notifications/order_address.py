"""Поиск сделки клиента в amoCRM по телефону.

До 16.09.2026 модуль ещё и сам ходил в amoCRM за адресом сделки прямо в
момент проведения заказа (`fetch_deal_address`, `resolve_order_address`).
С этой даты рабочий бот в amoCRM за адресом не ходит — адрес заказу
приносит связка админ-бота (решение владельца, ТЗ «адреса до конца»,
задача 3), и обе функции отсюда убраны.

Осталось то, что выбирает «ту самую» сделку клиента среди нескольких — это
правило не зависит от того, кто и когда его вызывает:

1. **Чужую сделку не берём.** Поиск amoCRM — подстрочный по всем полям, а у
   постоянного клиента открытых сделок бывает несколько (старая «ждёт оплаты»
   и сегодняшняя). Контакт сверяем по окончанию номера (`contact_matches_phone`),
   сделку выбираем по близости даты работы к моменту проведения
   (`pick_open_deal`), а не по свежести: «самая новая» на второй заказ
   подряд указала бы не на ту.
2. **Неверный адрес хуже отсутствующего.** Сделка не по адресу выглядит
   правдоподобно, и заметят её поздно — сомнительные сделки (`pick_open_deal`)
   лучше выбросить, чем взять.

Ни база, ни `bot.py` этот модуль не знает: на вход — контакты и сделки
amoCRM как словари, на выход — id нужной сделки или строка адреса из неё.
"""
