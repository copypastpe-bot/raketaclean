# AGENTS.md — raketaclean

<!--
  Адаптировано из CLAUDE.md для Codex, 2026-09-21.
  Общие правила Codex задаются глобальным AGENTS.md. Здесь — только правила
  этого репозитория. Codex подхватывает этот файл автоматически перед работой.
-->
...
Rules in this root file are common to the whole repository; `raketa-admin-bot/AGENTS.md`
holds the admin bot's own rules. Codex reads the applicable `AGENTS.md` files
automatically; when working outside the admin-bot directory, inspect only the
instructions relevant to the files being changed.
...
- Не управлять контекстом командой `/compact`: Codex делает сжатие контекста сам. Если контекст перестал быть достаточным для безопасного продолжения, закрыть сессию по правилам ниже, сохранив актуальные состояние и журнал.
...
- Разрешения Codex задаются активным профилем безопасности, а не `.claude/settings.json`. Для обычной работы нужны `pytest`, `git status/diff/log`, чтение файлов, `python -m` и скрипты проекта. `ssh` к серверу ботов допустим по правилам «Работа с сервером»; `scp` и `rsync` — только после согласия владельца.
- **Switches live apart from secrets** (owner decision 2026-09-21, applies to every
  ...
  touching a token.
- Если владелец явно разрешил субагентскую работу с worktree, после merge удалить worktree (`git worktree prune` и удаление ветки). Между сессиями остатков не оставлять.
...
5. применить правила контекстных файлов из глобального `AGENTS.md` Codex к обоим файлам.
